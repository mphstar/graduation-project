...
<div id="modalContentUpdate"
    ...
    <div class="bg-white p-8 w-full max-w-md mx-auto rounded-md shadow-md">
        <form method="POST" action="{{ route('update-student') }}">
            ...
            <div class="mt-4">
                <x-input-label for="update_student_id" :value="__('student ID')" />
                <x-text-input id="update_student_id" class="block mt-1 w-full" type="text" name="update_student_id"
                    :value="old('update_student_id')" required autocomplete="username" />
